 ...
 */
function getKey( domain, selector, dkimKeyRetriever, callback ) {

  if (dkimKeyRetriever) {
    dkimKeyRetriever(domain, selector, function(err, key) {
      if (err) {
        var keyError = new Error( 'No key for signature' )
        keyError.code = DKIM.PERMFAIL
        callback(keyError, null)
      }
      else {
        callback( null, new DKIM.Key({
          type: 'rsa',
          key: key
        }))
      }
