 ...
 */
function verifySignature( body, headers, dkimKeyRetriever, callback ) {
  ...
  try {
    ...
    if( !/^(DKIM-Signature)/i.test( headers[0] ) ) {
  ...
  DKIM.getKey( signature.domain, signature.selector, dkimKeyRetriever, ( error, key ) => {
