...
function CertVerifier(certBuffer, psesData) {
  ...
  try {
    ...
    if (certStr.startsWith(CERT_PEM_BEGIN)) {
      ...
      tmpBuffer = Buffer.from(certDerB64, 'base64');
      certASN1 = asn1js.fromBER(tmpBuffer.buffer.slice(tmpBuffer.byteOffset, tmpBuffer.byteOffset + tmpBuffer.byteLength));
    } else {
      certASN1 = asn1js.fromBER(this.certBuffer.buffer.slice(this.certBuffer.byteOffset, this.certBuffer.byteOffset + this.certBuffer.byteLength));
    }
  ...
  this._testCertIssuer = async function() {
    let intermediateBuffer = Buffer.from(CERT_INTERMEDIATE_BER, 'base64');
    let intermediateASN1 = asn1js.fromBER(intermediateBuffer.buffer.slice(intermediateBuffer.byteOffset, intermediateBuffer.byteOffset + intermediateBuffer.byteLength));
    let intermediateCert = new pkijs.Certificate({
      schema: intermediateASN1.result
    });
    let rootBuffer = Buffer.from(CERT_ROOT_BER, 'base64');
    let rootASN1 = asn1js.fromBER(rootBuffer.buffer.slice(rootBuffer.byteOffset, rootBuffer.byteOffset + rootBuffer.byteLength));
